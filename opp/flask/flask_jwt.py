...
def _default_request_handler():
    auth_header = current_app.config['JWT_AUTH_HEADER']
    auth_header_value = request.headers.get(auth_header, None)
...
def encode_token():
    return _jwt.encode_callback(_jwt.header_callback(),
                                _jwt.payload_callback())
...
class JWT(object):
    ...
    def authentication_handler(self, callback):
        """Specifies the identity handler function.
        This function receives two positional arguments. The first being
        the username the second being the password. It should return an
    ...
    def identity_handler(self, callback):
        """Specifies the identity handler function.
        This function receives one positional argument
    ...
    def auth_request_handler(self, callback):
        ...
        .. deprecated
        """
        warnings.warn("This handler is deprecated. The recommended approach "
                      "to have control over the authentication resource is to "
                      "disable the built-in  resource by setting "
                      "and registering your own authentication resource "
    ...
    def request_handler(self, callback):
        """Specifieds the request handler function.
        This function returns a JWT from the current
        request.
    ...
    def jwt_encode_handler(self, callback):
        """Specifies the encoding handler function.
        This function receives a payload and signs it.
    ...
    def jwt_payload_handler(self, callback):
        """Specifies the JWT payload handler function.
        This function receives the return value from
    ...
    def jwt_headers_handler(self, callback):
        """Specifies the JWT header handler function.
        This function receives the return value from
        the ``identity_handler`` function.
